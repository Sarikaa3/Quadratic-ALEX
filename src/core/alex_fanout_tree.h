...
double compute_level(const std::pair<T, P> values[], int num_keys,
  ...
  double c = node->model_.c_ * fanout;
  int left_boundary = 0;
  ...
  for (int i = 0; i < fanout; i++) {
    ...
    int c1 = c - (i + 1);
    double a1 = 2* a;
    int d = sqrt(b * b - 2 * a1 * c1); 
    right_boundary =
        i == fanout - 1
            ? num_keys
            : static_cast<int>(
                  std::lower_bound(values, values + num_keys, (- b + d) / a1,
    ...
    if (left_boundary == right_boundary) {
      used_fanout_tree_nodes.push_back(
          {level, i, 0, left_boundary, right_boundary, false, 0, 0, 0, 0, 0, 0});
      continue;
    }
    ...
    used_fanout_tree_nodes.push_back(
        {level, i, node_cost, left_boundary, right_boundary, false,
         stats.num_search_iterations, stats.num_shifts, model.a_, model.b_, model.c_, 
...
std::pair<int, double> find_best_fanout_bottom_up(
  ...
  fanout_tree.push_back(
      {{0, 0, best_cost, 0, num_keys, false, 0, 0, 0, 0, 0, num_keys}});
...
std::pair<int, double> find_best_fanout_top_down(
  ...
  while (true) {
    ...
    for (FTNode& tree_node : fanout_tree[fanout_tree_level - 1]) {
      ...
      int cm = c - (2 * tree_node.node_id + 1);
      double a2 = 2*a;
      int dm = sqrt(b * b - 2 * a2 * cm);
      ...
      QuadraticModel<T> node_models[2];
      for (int i = 0; i < 2; i++) {
      ...
      if (node_split_cost < tree_node.cost) {
        ...
        for (int i = 0; i < 2; i++) {
          new_level.push_back({fanout_tree_level, 2 * tree_node.node_id + i,
                               ...
                               node_models[i].b_, node_models[i].c_, 
...
int find_best_fanout_existing_node(const AlexModelNode<T, P>* parent,
  ...
  double cl = parent->model_.c_ - start_bucketID;
  double a1= 2.0 * parent->model_.a_;
  double a2= 4.0 * parent->model_.a_;
  double dl = sqrt(parent->model_.b_ * parent->model_.b_ - a2* cl);
  double cr = parent->model_.c_ - end_bucketID;
  double dr = sqrt(parent->model_.b_ * parent->model_.b_ - a2 * cr);
  double left_boundary_value =
		            ((-1.0 * parent->model_.b_ + dl) / (a1));
  double right_boundary_value =
			      (-1.0 * parent->model_.b_ + dr) / (a1);
  QuadraticModel<T> base_model;
  base_model.a_ = 1.0 / (right_boundary_value - left_boundary_value) * (right_boundary_value - left_boundary_value);
  base_model.b_ = -1.0 * 2 * base_model.a_ * left_boundary_value;
  base_model.c_ = base_model.a_ * left_boundary_value * left_boundary_value;
  ...
  for (int fanout = 1, fanout_tree_level = 0; fanout <= max_fanout;
    ...
    for (int i = 0; i < fanout; i++) {
      ...
      new_level.push_back({fanout_tree_level, i, node_cost, left_boundary,
                           ...
                           stats.num_shifts, model.a_, model.b_, model.c_, 
                           num_actual_keys});
    }
