...
class QuadraticModel {
 public:
  ...
  QuadraticModel(double a, double b, double c) : a_(a), b_(b), c_(c) {}
  explicit QuadraticModel(const QuadraticModel& other) : a_(other.a_), b_(other.b_), c_(other.c_) {}
  ...
  inline int predict_qua(T key) const {
//	  std::cout<<a_<<" "<<b_<<" "<<key<<std::endl;
    int pre = static_cast<int>(static_cast<double>(key) * (a_ * static_cast<double>(key) + b_) + c_);
    return pre;
  }
  inline int predict_linear(T key) const {
    return static_cast<int>(b_ * static_cast<double>(key) + c_);
  }
  inline double predict_double_qua(T key) const {
    return static_cast<double>(key) *(a_ * static_cast<double>(key) + b_) + c_;
  }
  inline double predict_double_lin(T key) const {
    return b_ * static_cast<double>(key) + c_;
  }
};
...
class QuadraticModelBuilder {
 public:
  QuadraticModel<T>* model_;

  explicit QuadraticModelBuilder<T>(QuadraticModel<T>* model) : model_(model) {}
  ...
  inline void add(T x, int y) {
    ...
    xxx_sum_ += static_cast<long double>(x) * x * x;
    xxxx_sum_ += static_cast<long double>(x) * x * x * x;
    xxy_sum_ += static_cast<long double>(x) * x * y;
  ...
  void build() {
    if (count_ <= 1) {
      model_->a_ = 0;
      model_->b_ = 0;
      model_->c_ = static_cast<double>(y_sum_);
      return;
    }
  sxx_ = (static_cast<long double>(count_) * xx_sum_ - x_sum_ * x_sum_);
  sxy_ = (static_cast<long double>(count_) * xy_sum_ - x_sum_ * y_sum_);
  sxx2_ = (static_cast<long double>(count_) * xxx_sum_ - xx_sum_ * x_sum_);
  sx2y_ = (static_cast<long double>(count_) * xxy_sum_ - xx_sum_ * y_sum_);
  sx2x2_ = (static_cast<long double>(count_) * xxxx_sum_ - xx_sum_ * xx_sum_);
  if (static_cast<double>(sxx_ * sx2x2_ - sxx2_ * sxx2_) == 0) {  
     model_->a_ = 0;
     model_->b_ = 0;
     model_->c_ = static_cast<double>(y_sum_) / count_;
     return;
  }
  auto a1 = static_cast<double>((sx2y_ * sxx_ - sxy_ * sxx2_) / (sxx_ * sx2x2_ - sxx2_ * sxx2_));
  auto b1 = static_cast<double>((sxy_ * sx2x2_ - sx2y_ * sxx2_) / (sxx_ * sx2x2_ - sxx2_ * sxx2_));
  auto c1 = static_cast<double>((y_sum_ / count_) - (static_cast<long double>(b1) * (x_sum_ / count_)) - (static_cast<long double>(a1) * (xx_sum_ / count_)));
  model_->a_ = a1;
  model_->b_ = b1;
  model_->c_ = c1;
  }
 ...
 private:
  ...
  long double xxx_sum_ = 0;
  long double xxxx_sum_ = 0;
  long double xxy_sum_ = 0;
  long double sxx_ = 0;
  long double sxy_ = 0;
  long double sxx2_ = 0;
  long double sx2y_ = 0;
  long double sx2x2_ = 0;
