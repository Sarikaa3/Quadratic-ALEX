...
// node's parent.
  ...
  forceinline data_node_type* get_leaf(
    ...
    while (true) {
      ...
      double bucketID_prediction = node->model_.predict_double_lin(key);
  ...
  data_node_type* get_leaf(
    ...
    while (!cur->is_leaf_) {
      auto node = static_cast<model_node_type*>(cur);
      int bucketID = node->model_.predict_linear(key);
      bucketID =
          std::min<int>(std::max<int>(bucketID, 0), node->num_children_ - 1);
      if (traversal_path) {
        traversal_path->push_back({node, bucketID});
 ...
 public:
  ...
  void bulk_load(const V values[], int num_keys) {
    ...
    QuadraticModel<T> root_data_node_model;
 ...
 private:
  ...
  void bulk_load_node(const V values[], int num_keys, AlexNode<T, P>*& node,
                      int total_keys,
                      const QuadraticModel<T>* data_node_model = nullptr) {
    ...
    if (best_fanout_tree_cost < node->cost_ ||
      ...
      for (fanout_tree::FTNode& tree_node : used_fanout_tree_nodes) {
        ...
        QuadraticModel<T> child_data_node_model(tree_node.a, tree_node.b, tree_node.c);
  ...
  data_node_type* bulk_load_leaf_node_from_existing(
    ...
    if (tree_node) {
      ...
      // recompute it
      QuadraticModel<T> precomputed_model(tree_node->a, tree_node->b, tree_node->c);
      ...
      QuadraticModel<T> precomputed_model(existing_node->model_);
  ...
  std::pair<Iterator, bool> insert(const T& key, const P& payload) {
    ...
    if (fail) {
      ...
      while (fail) {
        ...
        int bucketID = parent->model_.predict_linear(key);
 ...
 private:
  ...
  void expand_root(T key, bool expand_left) {
    ...
    T domain_size;
    if(istats_.key_domain_max_ > istats_.key_domain_min_){
      domain_size = istats_.key_domain_max_ - istats_.key_domain_min_;
    }
    else{
      domain_size = istats_.key_domain_min_ - istats_.key_domain_max_;
    }
    ...
    if (expand_left) {
      in_bounds_new_nodes_start =
          std::max(new_nodes_start, root->model_.predict_linear(new_domain_min));
    } else {
      in_bounds_new_nodes_end =
          std::min(new_nodes_end, root->model_.predict_linear(new_domain_max) + 1);
    }
  ...
  void create_two_new_data_nodes(data_node_type* old_node,
    ...
    int appending_right_bucketID = std::min<int>(
        std::max<int>(parent->model_.predict_linear(old_node->max_key_), 0),
        parent->num_children_ - 1);
    bool append_mostly_left = old_node->is_append_mostly_left();
    int appending_left_bucketID = std::min<int>(
        std::max<int>(parent->model_.predict_linear(old_node->min_key_), 0),
        parent->num_children_ - 1);
    ...
    while (right_boundary < old_node->data_capacity_ &&
           ...
           parent->model_.predict_linear(old_node->get_key(right_boundary)) <
               mid_bucketID) {
  ...
  data_node_type* split_upwards(
    ...
    if (append_mostly_right) {
      double appending_right_bucketID =
          parent->model_.predict_double_lin(leaf->max_key_);
    ...
    if (append_mostly_left) {
      double appending_left_bucketID =
          parent->model_.predict_double_lin(leaf->min_key_);
    ...
    if (parent->model_.predict_double_lin(key) < leaf_mid_bucketID) {
 ...
 public:
  ...
  int num_nodes() const {
    std::cout << "data_nodes  " << stats_.num_data_nodes << std::endl;
    std::cout << "model_nodes  " << stats_.num_model_nodes << std::endl;
