...
int main(int argc, char* argv[]) {
  ...
  cumulative_inserts += total_num_keys;

  double batch_lookup_time = 0.0;
  KEY_TYPE* lookup_keys = nullptr;
  if (lookup_distribution == "uniform") {
    lookup_keys = get_search_keys(keys, i, total_num_keys);
  } else if (lookup_distribution == "zipf") {
    lookup_keys = get_search_keys_zipf(keys, i, total_num_keys);
  } else {
  ...
  for (int j = 0; j < total_num_keys; j++){
  ...
  cumulative_lookups += total_num_keys;
  ...
  int a = index.num_nodes();
  long long dsize = index.data_size();
  long long msize = index.model_size();
  std::cout << "data_size  " << dsize << std::endl;
  std::cout << "model_size  " << msize << std::endl;
//  delete[] lookup_keys;
//  delete[] keys;
//  delete[] values;
}
